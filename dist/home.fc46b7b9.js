...
function fetchFiles(func) {
  fetch("/fs/getPaginated").then(function (response) {
...
function updateFiles(files) {
  ...
  for (var i = 0; i < files.length; i++) {
    newFilesDiv += "<tr>" + '<td><span class="fileItem"><a href="/fs/downloadFileById?id=' + files[i].Id + '" target="_blank">' + files[i].FileName + "</a></td>" + "<td>" + '<img src="./resources/delete_icon.png" width="25" height="25" onclick="deleteFileById(' + files[i].Id + ')"></span></td>' + "</tr>";
  }
...
function getOriginIp(func, filestruct, afterFunc) {
  fetch("/fs/localIp").then(function (response) {
...
function deleteFileById(id) {
  ...
  fetch("/fs/deleteFileById?id=" + id).then(function (response) {
    return response.text();
  }).then(function (text) {
    console.log(text);
    fetchFiles(updateFiles);
...
function fileTransferByParts(origin, filestruct, afterFunc) {
  ...
  exampleSocket.onopen = function () {
    // if()
    exampleSocket.send(JSON.stringify(filestruct)); // exampleSocket.
  };
  ...
  exampleSocket.onmessage = function (event) {
    switch (event.data) {
      case "SEND_DATA":
        ...
        progressCompletedPercentage = index * PACKET_LIMIT * 100 / filestruct.size;
...
function sendData(filestruct, websocketConn, index) {
  ...
  messagePart = filestruct.content.slice(index * PACKET_LIMIT, index * PACKET_LIMIT + size);
